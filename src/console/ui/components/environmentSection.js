...
export function EnvironmentSection({ status, loading, capabilities }) {
  const caps = capabilities || {};
  const knownKeys = status.env?.knownKeys || [];
  const envList = knownKeys.length
    ? knownKeys.map((name) => `<li class="py-0.5">${name}</li>`).join("")
    : "<li class='text-slate-500'>No env names detected</li>";
  const addBusy = Boolean(loading.addEnvBtn);
  const canManageEnv = caps.env_management !== false;
  ...
  return `
    <section class="rounded-xl bg-white p-4 shadow-sm">
      ...
      ${canManageEnv
        ...
        : `<p class="mt-3 text-sm text-slate-500">Environment management is disabled for this project.</p>`}
    </section>
  `;
}
