...
function mark(summary: Summary, level: Level): void {
  summary[level] += 1;
}
...
async function checkCommand(
  ...
  try {
    ...
    fail(
      ...
      result.stderr.trim() || result.stdout.trim() || "No details available.",
    );
    mark(summary, "blocker");
    return { ok: false };
  } catch (error) {
    fail(
      `${label}: unavailable or blocked`,
...
async function checkEnvFiles(summary: Summary, projectRoot: string): Promise<void> {
...
export async function doctor(): Promise<number> {
  const summary: Summary = { ok: 0, warn: 0, blocker: 0 };
  ...
  if (vercelVersion.ok) {
    try {
      ...
      if (whoami.exitCode === 0) {
        ...
        warn(
          "Vercel auth: not logged in",
          whoami.stderr.trim() || "Run `vercel login` to authenticate.",
        );
        mark(summary, "warn");
      }
    } catch (error) {
      warn(
        "Vercel auth check skipped",
  ...
  const supabaseVersion = await checkCommand(
    summary,
    "Supabase CLI",
    "supabase",
    ["--version"],
